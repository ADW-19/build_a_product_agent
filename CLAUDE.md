...
5. **示例代码**：均为 Python 3.13+。Web 层用 FastAPI（async def 路由），Agent 编排用 LangGraph（`StateGraph`/`astream_events`/checkpointer），工具层用 LangChain（`@tool`），数据校验用 Pydantic v2，Redis 一律用 `redis.asyncio`，LLM 客户端用 `AsyncOpenAI`。
   ...
   - **禁止的已弃用写法**（`scripts/check_snippets.py` 的 `BANNED` 清单会拦，改脚本时两处要保持一致）：`set_entry_point`/`set_finish_point`、`langgraph.prebuilt.create_react_agent`、`from langchain.chains|retrievers|indexes`、`MemorySaver`、`checkpointer=RedisSaver.from_conn_string(...)`（它是 `@contextmanager`）、`await httpx.post(...)`、`await xxx.invoke(...)`、pymilvus 的 `connections.connect`/`utility.has_collection`、A2A 的 `tasks/send` 与 `/.well-known/agent.json`、Cohere `rerank-multilingual-v3.0`/`rerank-english-v3.0`、Langfuse v2 的 `from langfuse.callback import` 与 `langfuse.trace(`、`RagasEvaluator`。注意这里只收"照抄必然出错"的写法：`/health_generate` 在 SGLang 示例里是正确端点，因此不入清单。
...
- **提交前必做**：`pip install pyyaml && python scripts/check_snippets.py`，必须 **0 个问题**。它只做四条能明确定性的检查：Python 块语法、YAML 可解析、围栏语言标注、已弃用 API 残留。**故意不报"建议类"告警**——本仓库示例统一引用 `core/` 骨架，跨文件占位符号是刻意为之，做成告警只会训练人忽略输出。校验器只扫代码、不扫注释，所以"❌ 错误示范"可以放心写在注释里。`BANNED` 清单会过期，复核版本基线时一并增删。
